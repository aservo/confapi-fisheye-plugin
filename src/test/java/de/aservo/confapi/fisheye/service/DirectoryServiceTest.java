...
import de.aservo.confapi.fisheye.model.util.DirectoryBeanUtil;
...
public class DirectoryServiceTest {
    ...
    public void testDeleteDirectories() throws DirectoryCurrentlySynchronisingException {
        Directory directory = createDirectory();
        doReturn(directory).when(crowdDirectoryService).findDirectoryById(anyLong());
        doReturn(Collections.singletonList(directory)).when(crowdDirectoryService).findAllDirectories();

        directoryService.deleteDirectories(true);

        verify(crowdDirectoryService).removeDirectory(anyLong());
    }
    ...
    public void testDeleteDirectoriesWithoutInternalDirectory() throws DirectoryCurrentlySynchronisingException {
        ...
        verify(crowdDirectoryService, never()).removeDirectory(1L);
    }
    ...
    private Directory createDirectory(String url, DirectoryType directoryType) {
        ...
        attributes.put(SYNC_GROUP_MEMBERSHIP_AFTER_SUCCESSFUL_USER_AUTH_ENABLED, "NEVER");


        InternalEntityTemplate entityTemplate = new InternalEntityTemplate(1L, "test", true, new Date(), new Date());
        DirectoryImpl directory = new DirectoryImpl(entityTemplate);
